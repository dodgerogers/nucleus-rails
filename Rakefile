...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("nucleus-rails.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in nucleus-rails.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "nucleus-rails.gemspec", /ruby_version = .*">= (.*)"/ => lowest
